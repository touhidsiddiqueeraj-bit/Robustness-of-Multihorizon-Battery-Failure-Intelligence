...
\textbf{Practical takeaway for operators.} Deploying a lab-trained hazard model directly into field operation risks systematically overconfident failure probabilities---the calibration error jumps from near-zero to 0.27--0.38 under partial cycling (Section~\ref{sec:results}). However, the fix does not require retraining the base model. An operator need only collect 50--100 operational cycles, extract the same per-cycle features, run the frozen model, and fit a new calibrator (isotonic or Platt) on the resulting raw probabilities. This recalibration step recovers 71--88\% of the calibration gap (ECE drops to 0.06--0.09), and evidence suggests the particular calibrator choice does not matter (Section~\ref{sec:recal}). The associated table (Table~\ref{tab:recal_summary}) serves as a deployment checklist: which operating regime is safe, which requires recalibration, and which is unsafe even after recalibration.
...
We also evaluate Platt (sigmoid) scaling as an alternative to isotonic regression. The results are near-identical: Platt achieves ECE of 0.06--0.08 (Table~\ref{tab:recal_summary}, column~4), overlapping with isotonic at every horizon. The standard deviations are somewhat larger for Platt (e.g.\ 0.037 at \(H=20\) vs.\ 0.027 for isotonic), suggesting slightly higher seed-to-seed variance, but the means are indistinguishable. This confirms that it is recalibration itself that matters, not the particular functional form of the calibrator.
...
\midrule
10 & 0.010 & 0.297$\pm$0.017 & 0.067$\pm$0.022 & 0.064$\pm$0.013 \\
20 & 0.031 & 0.286$\pm$0.012 & 0.067$\pm$0.027 & 0.083$\pm$0.037 \\
30 & 0.013 & 0.299$\pm$0.014 & 0.081$\pm$0.024 & 0.079$\pm$0.036 \\
50 & 0.023 & 0.362$\pm$0.022 & 0.086$\pm$0.028 & 0.076$\pm$0.036 \\
\bottomrule
\end{tabular}
\end{table}
